...
bool TextureManager::AddTexture(const std::string& relativePath) {
    if (HasTexture(relativePath)) {
        Debug::Log("[[検出]] 取得済みのTexture : " + relativePath,MESSAGE_COLOR::YELLOW);
        return false;
    }
    ...
    Debug::Log("[[成功]] AddTexture  : " + fullPath,MESSAGE_COLOR::GREEN);
...
std::shared_ptr<Texture> TextureManager::GetTexture(const std::string& relativePath) {
    ...
    if (it != m_TextureList.end()) {
        Debug::Log("[[成功]] GetTexture  : " + relativePath, MESSAGE_COLOR::GREEN);
        return it->second;
    }
