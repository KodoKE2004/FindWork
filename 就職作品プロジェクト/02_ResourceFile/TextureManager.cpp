...
bool TextureManager::AddTexture(const std::string& relativePath) {
    if (HasTexture(relativePath)) {
        Debug::Log("[[検出]] 取得済みのTexture : " + relativePath,MessageColor::Yellow);
        return false;
    }
    ...
    Debug::Log("[[成功]] AddTexture : " + relativePath,MessageColor::Green);
...
std::shared_ptr<Texture> TextureManager::GetTexture(const std::string& relativePath) {
    ...
    if (it != m_TextureList.end()) {
        // Debug::Log("[[成功]] GetTexture : " + relativePath, MessageColor::Green);
        return it->second;
    }
