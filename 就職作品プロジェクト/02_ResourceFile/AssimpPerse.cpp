...
namespace AssimpPerse
{
	...
	//
	std::string ResolveTexturePath(const std::string& directory, const aiString& texPath) {
		...
		// 実ディレクトリと結合して再構成
		std::string resolved = directory + "/" + filename;

		// ファイル存在チェック
		std::ifstream ifs(resolved);
		if (!ifs) {
			std::cerr << "Texture not found: " << resolved << std::endl;
		}
		return resolved;
	}
			...
			for (unsigned int t = 0; t < material->GetTextureCount(aiTextureType_DIFFUSE); t++)
			{
				...
				if (AI_SUCCESS == material->Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, t), path))
				{
					...
					if (texpath.find(':') != std::string::npos) {
						//スラッシュ又はバックスラッシュが最後に現れる位置を探す
						size_t pos = texpath.find_last_of("/\\");
						if (pos != std::string::npos) {
							//最後のスラッシュ又はバックスラッシュの次から文字列を返す
					...
					if (auto tex = pScene->GetEmbeddedTexture(path.C_Str())) {
						...
						bool sts = texture->LoadFromFile(texname);
						if (sts) {
							g_textures[m] = std::move(texture);
						}
