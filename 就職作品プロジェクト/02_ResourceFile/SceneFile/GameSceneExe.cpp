...
void GameSceneExe::Update(float tick)
{
    ...
    // 経過した8分音符数を、1小節の拍数で割る。
    m_CurrentMeasure =
        rhythmResult.currentBeat /
        rhythmBeat.GetBeatConst().m_BeatsPerBar;

    // 残り拍を四分音符単位へ変換する。
    const int restQuarterBeat =
        (rhythmBeat.GetBeatTotal() -
         rhythmResult.currentBeat) / 2;
    ...
    for (int i = previousQuarterBeat; i < currentQuarterBeat; ++i)
    {
        ...
        if (restQuarterBeat < 4)
        {
            // 残り 3,2,1,0 ではカウントダウン演出を出す。
            m_Bomber->SetCount(restQuarterBeat);
            m_Bomber->CountDown();
            ...
            else if (restQuarterBeat < 4 && restQuarterBeat > 0)
            {
